...
//  FPS MONITOR FOR PERFORMANCE TESTING
  ...
  SFB.GameView.prototype.bindFlameThrower = function (key, gnomeId) {
    ...
    window.game.gnomes[gnomeId - 1].wasKeyPressed;
    if(window.key.isPressed(key)) {
      ...
      if (window.flamers[gnomeId] === undefined ) {
        window.flamers[gnomeId] = new SFB.FartFlame({ gnomedId: gnomeId, gnome: window.game.gnomes[gnomeId - 1] })
      }
    }
    ...
    setTimeout(function () {
      ...
      if (window.flamers[gnomeId]) {
        window.flamers.splice(gnomeId, 1)
      }
    }, window.game.delta)
  }
