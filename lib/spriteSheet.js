(function () {
  if (typeof SFB === "undefined") {
    ...
    SpriteSheet.prototype.draw = function (position, origin, sheetIndex, mirror) {
        ...
        SFB.Game.drawImage(this._image, position, 0, 1, origin, imagePart, mirror);
    };

    SpriteSheet.prototype.drawFrame = function (sheetIndex, ctx, xpos, ypos) {
      var columnIndex = sheetIndex % this._sheetColumns;
      var rowIndex = Math.floor(sheetIndex / this._sheetColumns) % this._sheetRows;
      var imagePart = new SFB.Rectangle(columnIndex * this.width, rowIndex * this.height, this.width, this.height);
      SFB.Game.prototype.drawImageTwo(this._image, ctx, xpos, ypos, imagePart)
    };

    // SpriteSheet.prototype.drawSprite = function (sprite, sourceRect) {
    //   var canvasScale = this.scale;
    //   var rotation = typeof rotation !== 'undefined' ? rotation : 0;
    //   var scale = 1;
    //   var origin = SFB.Vector2.zero;
    //   var sourceRect = typeof sourceRect !== 'undefined' ? sourceRect : new SFB.Rectangle(0, 0, sprite.width, sprite.height);
    //   SFB.Game.drawImageTwo(
    //           this.sprite, sourceRect.x, sourceRect.y,
