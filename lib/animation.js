(function() {
  ...
  var Rectangle = SFB.Rectangle = function (x, y, w, h) {
    ...
    SFB.Animation.prototype.explosion = function (ctx, xpos, ypos) {
        var canvasScale = SFB.GameView.prototype.scale;

        var newXpos, newYpos;

        if (xpos >= 960) {
          newXpos = (xpos % 960) * canvasScale.x
        } else {
          newXpos = -(xpos) * canvasScale.x
        }

        if (ypos >= 540) {
          newYpos = (ypos % 540) * canvasScale.y
        } else {
          newYpos = (-ypos) * canvasScale.y
        }
        ...
        var emitter	= Fireworks.createEmitter({nParticles : 20})
          ...
          .effectsStackBuilder()
            .spawnerOneShot(8)
            .position(Fireworks.createShapeSphere(newXpos, newYpos, 0, 1))
            .velocity(Fireworks.createShapeSphere(newXpos, newYpos, 0, 60))
            .lifeTime(0.7, 1)
            .renderToCanvas({
               ctx	: ctx,
              type	: 'drawImage',
    ...
    SFB.Animation.prototype.explosionStart = function (ctx) {
