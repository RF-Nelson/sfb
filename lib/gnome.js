(function() {
  ...
  SFB.Gnome = function (options) {
    ...
    this.flaming = false;
    this.flamers = [];
    this.livesPos = options.livesPos;
    this.dead = false;
  ...
  SFB.Gnome.prototype.fireSpecialFart = function() {
    ...
    if (this.specialFart === "Bounce") {
      var fart = new SFB.FartBounce({ gnome: this, xpos: this.xpos, ypos: this.ypos, xvel: xvel, yvel: this.yvel, game: this.game, gnome_id: this.id, team: this.team, strength: 3 });
    } else if (this.specialFart === "Flame") {
      var fart = new SFB.FartFlame({ xpos: this.xpos, ypos: this.ypos, game: this.game, gnome_id: this.id, team: this.team, gnome: this, strength: 1, key: this.key, yvel: this.yvel });
    } else if (this.specialFart === "Mine") {
      var fart = new SFB.FartMine({ xpos: this.xpos, ypos: this.ypos, xvel: xvel, game: this.game, gnome_id: this.id, team: this.team, gnome: this, strength: 25 });
    }
  ...
  SFB.Gnome.prototype.die = function () {
    ...
    this.xpos = -100;
    this.dead = true;
    this.game.numDeaths += 1;
    /* Flash message of gnomes death */
  };
  ...
  SFB.Gnome.prototype.drawHealthMeter = function(ctx){
    ...
    SFB.Animation.prototype.roundRect(ctx, this.xpos - 25, this.ypos - 160, 130, 50, "white")

    SFB.Animation.prototype.roundRect(ctx, this.xpos - 20, this.ypos - 155, 120, 40, "lightcoral")

    SFB.Animation.prototype.roundRect(ctx, this.xpos - 20, this.ypos - 155, 120 * (this.healthMeter / 100), 40, "red")

    ctx.font="24px Delius Unicase";
    ctx.fillStyle = "white"
    ctx.fillText("HEALTH", this.xpos - 9, this.ypos - 125);
  ...
  SFB.Gnome.prototype.drawBeanMeter = function(ctx){
    ...
    SFB.Animation.prototype.roundRect(ctx, this.xpos - 25, this.ypos - 215, 130, 50, "white")

    SFB.Animation.prototype.roundRect(ctx, this.xpos - 20, this.ypos - 210, 120, 40, "black")

    if (this.beanMeter > 0) {
      SFB.Animation.prototype.roundRect(ctx, this.xpos - 20, this.ypos - 210, 120 * (this.beanMeter / 100), 40, "brown")
    }

    ctx.font="24px Delius Unicase";
    ctx.fillStyle = "white"
    ctx.fillText("BEANS", this.xpos, this.ypos - 181);
