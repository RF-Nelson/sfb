(function() {
  ...
  if (this.onGround) {
    ...
    if (this.airFacing === "Left") {
      var newXvel = (this.xvel + 1);
      this.xvel = newXvel > 15 ? 10 : newXvel;
      this.airFacing = "Right";
    } else {
      var newXvel = (this.xvel + 1);
      this.xvel = newXvel > 15 ? 10 : newXvel;
      this.airFacing = "Right";
...
SFB.Gnome.prototype.runLeft = function () {
  ...
  SFB.Gnome.prototype.jump = function () {
    /* Set the in air facing direction eaual to the current facing direction. This will allow for later compensation when facing one direction in the air and trying to move the other way. This change in direction and velocity while in the air would not be possible according to natural physics so we will add greater acceleration in the oppoiste direction to achieve this unnatural movement.*/
    this.airFacing = this.facing;
  ...
  SFB.Gnome.prototype.draw = function(ctx) {
    ...
    } else if ((this.yvel !== 0) || (this.yvel === 0 && this.ypos !== 669)) {
