(function() {
  ...
  SFB.Gnome.prototype.runRight = function () {
    ...
    if (this.isFireFarting) {
      if (window.FireBalls[this.id -1 ].Left.effect._started) {
        window.FireBalls[this.id -1 ].Left.effect.effect('spawner').opts.stop()
      }
      window.FireBalls[this.id -1 ].Right.effect.effect('spawner').opts.start()
    }
  };
  ...
  SFB.Gnome.prototype.runLeft = function () {
    ...
    if (this.isFireFarting) {
      if (window.FireBalls[this.id -1 ].Right.effect._started) {
        window.FireBalls[this.id -1 ].Right.effect.effect('spawner').opts.stop()
      }
      window.FireBalls[this.id -1 ].Left.effect.effect('spawner').opts.start()
    }
  ...
  SFB.Gnome.prototype.move = function() {
    ...
    if (window.FireBalls[this.id - 1].Right.effect) {
      // if (this.facing === "Left") {
        window.FireBalls[this.id - 1].Left.effect.effectsStackBuilder().position(Fireworks.createShapeSphere(
          this.xpos + 14,
          this.ypos -54,
          0,
          10))
      // } else {
        window.FireBalls[this.id - 1].Right.effect.effectsStackBuilder().position(Fireworks.createShapeSphere(
          this.xpos -75,
          this.ypos -54,
          0,
          10))
      ...
      //   if (this.face === "Left") {
      //     window.FireBalls[this.id - 1].effect.effectsStackBuilder().velocity(Fireworks.createShapeSphere(-500, 0, 0, 150))
      //   } else {
      //     window.FireBalls[this.id - 1].effect.effectsStackBuilder().velocity(Fireworks.createShapeSphere(500, 0, 0, 150))
      //   }
  ...
  SFB.Gnome.prototype.fireFart = function() {
    ...
    this.beanMeter -= 2;
    var fart = new SFB.Fart({ gnome: this, xpos: this.xpos, ypos: this.ypos, xvel: xvel, yvel: this.yvel, game: this.game, gnome_id: this.id, strength: 2 });
  };
  ...
  SFB.Gnome.prototype.fireSpecialFart = function() {
    ...
    if (this.specialFart === "Bounce") {
      var fart = new SFB.FartBounce({ xpos: this.xpos, ypos: this.ypos, xvel: xvel, yvel: this.yvel, game: this.game, gnome_id: this.id, strength: 5 });
    } else if (this.specialFart === "Flame") {
      var fart = new SFB.FartFlame({ game: this.game, gnome_id: this.id, gnome: this, strength: 3 });
    } else if (this.specialFart === "Mine") {
      var fart = new SFB.FartMine({ xpos: this.xpos, ypos: this.ypos, xvel: xvel, game: this.game, gnome_id: this.id, gnome: this, strength: 15 });
    }

  };


  SFB.Gnome.prototype.handleHit = function (strength) {
  ...
  SFB.Gnome.prototype.handleFartPack = function () {
    this.fartPackMeter =  this.fartPackMeter > 99 ? this.fartPackMeter = 100 : this.fartPackMeter += 25
  };
  ...
  // SFB.Gnome.prototype.drawFartMeter = function(ctx){
  //   var canvasScale = SFB.GameView.prototype.scale;
  //   scale = typeof scale !== 'undefined' ? scale : 1;
  //
  //   ctx.save();
  ...
  //   ctx.beginPath();
  //   ctx.fillStyle = "white";
  //   ctx.fillRect( this.xpos - 25, this.ypos - 220, 130, 30 );
  //   ctx.fill();
  //   ctx.closePath();
  //
  //   ctx.beginPath();
  //   ctx.fillStyle = "lightcoral";
  //   ctx.fillRect( this.xpos - 20, this.ypos - 215, 120, 20 );
  //   ctx.fill();
  //   ctx.closePath();
  //
  //   ctx.beginPath();
  //   ctx.fillStyle = "red";
  //   ctx.fillRect( this.xpos - 20, this.ypos - 215, 120 * (this.fartPackMeter / 100), 20 );
  //   ctx.fill();
  //   ctx.closePath();
  //
  //
  ...
  //   ctx.fillStyle = "black"
  //   ctx.fillText("Fart Pack", this.xpos - 8, this.ypos - 198);
  //
  //   // DRAW GNOME HITBOX
  //   // var gnomeHitBox = this.hitBox()
  //   // ctx.strokeRect(gnomeHitBox.x, gnomeHitBox.y, gnomeHitBox.width, gnomeHitBox.height)
