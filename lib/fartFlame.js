(function() {
  ...
  SFB.FartFlame.prototype.hitBox = function () {
    if (this.gnome.facing === "Left") {
      return new SFB.Rectangle(this.gnome.xpos + 90, this.gnome.ypos - 40,
        280, 120)
    } else {
      return new SFB.Rectangle(this.gnome.xpos - 300, this.gnome.ypos - 40,
        350, 120)
    }
  ...
  SFB.FartFlame.prototype.FlameThrower = function () {
    ...
    this.key.wasKeyPressed;
    if(window.key.isPressed(this.key)) {
      if (fireBalls !== undefined && !gnome.wasKeyPressed) {
        if (facing === "Left") {
      ...
      gnome.wasKeyPressed = true
      gnome.isFireFarting = true
      gnome.isFarting = 11;
    }
    setTimeout(function () {
      if(!window.key.isPressed(this.key) && fireBalls !== undefined && (fireBalls.Right.effect._started || fireBalls.Left.effect._started)) {
        if (facing === "Left") {
          fireBalls.Left.effect.effect('spawner').opts.stop()
        } else {
          fireBalls.Right.effect.effect('spawner').opts.stop()
        }
        gnome.wasKeyPressed = false
        gnome.isFireFarting = false
      }
    }, gnome.game.delta)
  }
