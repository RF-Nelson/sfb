...
// and draw themselves.
SFB.FartCloud = function (options) {

  options.xvel = options.xvel > 0 ? 2*options.xvel : 10;
  options.yvel = 0;
	options.radius = 10;
  options.color = 'grey';
  SFB.Fart.call(this, options);
  // this.strength = options.strength;
  this.wrappable = false;
  this.game.farts.push(this);
  this.gnome_id = options.gnome_id;
  this.fartCircles = SFB.FartCloud.prototype.makeFartCircles(options)
};

  SFB.Utils.inherits(SFB.FartCloud, SFB.Fart)

SFB.FartCloud.prototype.makeFartCircles = function (options) {
  ...
  deltas.forEach(function (delta) {
    arrayOfFartCircles.push(new SFB.FartCloud({
      xpos: options.xpos + delta[0], ypos: options.ypos + delta[1],
      xvel: options.xvel, yvel: options.yvel,
      game: options.game, gnome_id: options.id
    }))
  })
