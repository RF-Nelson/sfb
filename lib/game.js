(function () {
  ...
  var Game = SFB.Game = function (numPlayers) {
    ...
    this.playerStartingSettings = [
      { gnomeId: 1, xpos: 100, ypos: (SFB.Game.DIM_Y - 131), color: "blue", facing: "Right" },
      { gnomeId: 2, xpos: 1640, ypos: (SFB.Game.DIM_Y - 131), color: "orange", facing: "Left" },
      { gnomeId: 3, xpos: 300, ypos: (SFB.Game.DIM_Y - 131), color: "purple", facing: "Right" },
      { gnomeId: 4, xpos: 1340, ypos: (SFB.Game.DIM_Y - 131), color: "green", facing: "Right" }
    ];
  ...
  Game.prototype.addGnome = function (gnome, gnomeIndex) {
    ...
    var newGnome = new SFB.Gnome({game: this, xpos: gnomeBasic.xpos, ypos: gnomeBasic.ypos, id: gnomeBasic.gnomeId, color: gnomeBasic.color, facing: gnomeBasic.facing, onGround: true, speed: gnomeSpeed, toughness: gnomeToughness, team: gnome.attrs.playerTeam, specialFart: gnome.attrs.fart });
  ...
  Game.prototype.draw = function (ctx) {
    ...
    ctx.fillText("Collect Beans To Use Special Fart", 400, 50);
    ...
    ctx.fillText("Player1 Beans: " + window.game.gnomes[0].beanCount, 15, 50);
    ctx.fillText("Player2 Beans: " + window.game.gnomes[1].beanCount, 830, 50);
    if (window.game.gnomes[0].beanCount >= 1) {
      ctx.fillText("Press 'f' for fart pack.", 15, 100);
    }

    if (window.game.gnomes[1].beanCount >= 1) {
      ctx.fillText("Press 'down' for fart pack", 800, 100);
    }

    if (window.game.gnomes[0].fartPacking) {
      ctx.fillText("Fart Pack Time: " + window.game.gnomes[0].fartPackingTimer, 15, 130);
      ctx.fillText("You can fuck while fart packing.", 15, 160);


    } else if (window.game.gnomes[1].fartPacking) {
      ctx.fillText("Fart Pack Time: " + window.game.gnomes[1].fartPackingTimer, 830, 130);
      ctx.fillText("You can jump while.", 830, 160);
      ctx.fillText("fudge packing.", 850, 190);
    }
    ...
    var flamethrowerCanvas = document.getElementById('flamethrower');
     if (flamethrowerCanvas && window.FireBalls.length > 0) {
      ...
      for (var i = 0; i < window.game.gnomes.length; i++) {
        if (FireBalls[i].Left.effect && (FireBalls[i].Left.effect._started)) {
          // if (window.game.gnomes[i].facing === "Left") {
            FireBalls[i].Left.effect.update(window.game.delta/1000).render()
        }
        if (FireBalls[i].Right.effect && (FireBalls[i].Right.effect._started)) {
          // if (window.game.gnomes[i].facing === "Left") {
          FireBalls[i].Right.effect.update(window.game.delta/1000).render()
        }
      }
