(function () {
  ...
  Game.prototype.gameOver = function () {
    ...
    if (GAMEMODE === "Free") {
          winner = "<li id='winner'>Player " + winner.id + " Wins!</li>";
      $('.closing-screen-free')[0].classList.remove('hidden');

      $('.closing-items').append(winner);

      this.gnomes.forEach( function(gnome){
        var stats = "<ul class='player-" + gnome.id + "'<li id='shots'>Farts Fired: " + gnome.shots + "</li><li id='hits'>Fart Hits: " + Math.floor(gnome.hits) + "</li><li id='accuracy'>Accuracy " + Math.round((gnome.hits / gnome.shots) * 100) / 100 + "</li></ul>"
        $('.individual-player-stats').append(stats);
      });
    } else {
      $('.closing-screen-team')[0].classList.remove('hidden');
      winningTeam = "<li id='winningTeam'>Team " + winningTeam + " Wins!</li>";

        $('.closing-items').append(winningTeam);
